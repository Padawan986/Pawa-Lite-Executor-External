...
appdata = Path(os.environ['APPDATA']+'\\Pawa-Lite-BetaV2')
offsets_to_copy = {
...
def update(version):
    ...
    ua = {'User-Agent': 'Pawa-Lite(Beta)V2'}
    offsets = requests.get(f"https://offsets.imtheo.lol/{version}/offsets.json", headers=ua, timeout=15)
    fflags = requests.get(f"https://offsets.imtheo.lol/{version}/fflags.json", headers=ua, timeout=15)
    try:
        j = offsets.json()
        jf = fflags.json()
    ...
    if os.path.exists(appdata / 'offset_cache.json'):
        with open(appdata / 'offset_cache.json', 'r') as f:
            ...
            if 'schema' not in j or j['schema'] != 2:
                ...
                # Same version, but the dumpers may have completed the data
                # since (ModuleScript.ByteCode is 0 until reversed).
                offs = j.get("offsets", {})
                if not offs.get("module_bytecode"):
                    print("Bytecode offsets still missing, re-checking provider")
                    upd = True
    else:
